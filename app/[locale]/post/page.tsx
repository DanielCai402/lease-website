          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label={t('fields.availableFrom')} required error={errors.availableFrom}>
              <input
                ...
                max={form.availableTo ? prevDay(form.availableTo) : undefined}
                onChange={(e) => {
                  const val = e.target.value;
                  if (form.availableTo && val >= form.availableTo) {
                    setForm((f) => ({ ...f, availableFrom: val, availableTo: '' }));
                    setErrors((err) => ({ ...err, availableFrom: t('validation.dateRangeFrom'), availableTo: '' }));
                  } else {
                    setForm((f) => ({ ...f, availableFrom: val }));
                    setErrors((err) => ({ ...err, availableFrom: '' }));
                  }
                }}
            ...
            <Field label={t('fields.availableTo')} required error={errors.availableTo}>
              <input
                ...
                min={form.availableFrom ? nextDay(form.availableFrom) : undefined}
                onChange={(e) => {
                  const val = e.target.value;
                  setForm((f) => ({ ...f, availableTo: val }));
                  if (form.availableFrom && val && val <= form.availableFrom) {
                    setErrors((err) => ({ ...err, availableTo: t('validation.dateRange') }));
                  } else {
                    setErrors((err) => ({ ...err, availableTo: '' }));
                  }
                }}
